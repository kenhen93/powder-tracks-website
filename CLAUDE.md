...
- **Day details (free, on-device):** the resort is auto-detected
  ...
  **optimized copies with all hidden metadata (including GPS) stripped —
  ...
  `images/screenshots/free/day-details.png`.
...
- Assets come from the app repo's `assets/images/` (snowflake mark with
  ...
  in `images/social/`, app screenshots in `images/screenshots/` — split
  by tier into `free/` and `premium/` subfolders (mockups in the
  gitignored `images/examples/`). Copy new assets into the matching
